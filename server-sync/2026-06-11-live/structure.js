...
function PersonNode({ id, data, selected }) {
  var name = data.name || '未知成员';
  var role = data.role || '';
  var initials = name.charAt(0).toUpperCase();
  return html`
    <div className=${'gp-rf-person' + (selected ? ' selected' : '')}>
      <div className="gp-rf-person-avatar">
        ${data.avatar
          ? html`<img src=${data.avatar} alt=${name} />`
          : html`<span className="gp-rf-person-initials">${initials}</span>`
        }
      </div>
      <div className="gp-rf-person-info">
        <div className="gp-rf-person-name">${name}</div>
        ${role ? html`<div className="gp-rf-person-role">${role}</div>` : ''}
      </div>
      <${RFHandle} type="source" position=${RFPosition.Top} className="person-handle" />
      <${RFHandle} type="target" position=${RFPosition.Bottom} className="person-handle" />
    </div>
  `;
}

const NODE_TYPES = { module: GPNode, root: RootNode, person: PersonNode };
...
async function loadStructureFromAPI(projectCode) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      applyGraphData(data.data || data, projectCode);
